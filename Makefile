# rust-graph-algorithms — c12 Graph Algorithms reference workspace.
#
# Idiomatic make targets that mirror what `gate` runs in CI. On a
# developer machine, converge on these — if `make ship-ready` is
# green, the workspace is mergeable.
#
# Linted by `bashrs lint Makefile` in CI.

.SUFFIXES:
.DELETE_ON_ERROR:
.ONESHELL:

CARGO ?= cargo
RUSTDOCFLAGS ?= -D warnings
RUSTFLAGS ?= -D warnings

.PHONY: help build test test-doc lint fmt fmt-check doc coverage \
	audit deny release contracts contracts-check \
	bashrs-lint comply clean ship-ready

help:
	@echo "rust-graph-algorithms — make targets"
	@echo ""
	@echo "  build          cargo build --workspace"
	@echo "  test           cargo test --workspace --all-targets"
	@echo "  test-doc       cargo test --workspace --doc"
	@echo "  fmt            cargo fmt --all"
	@echo "  fmt-check      cargo fmt --all -- --check (CI gate)"
	@echo "  lint           cargo clippy -D warnings"
	@echo "  doc            cargo doc with -D warnings"
	@echo "  coverage       cargo llvm-cov --fail-under-lines 100"
	@echo "  audit          cargo audit --deny warnings"
	@echo "  deny           cargo deny check"
	@echo "  release        cargo build --release --workspace"
	@echo "  contracts-check  pv lint contracts/"
	@echo "  bashrs-lint    bashrs lint Makefile"
	@echo "  comply         pmat comply"
	@echo "  ship-ready     full local CI gate"
	@echo "  clean          cargo clean"

build:
	$(CARGO) build --workspace

test:
	$(CARGO) test --workspace --all-targets

test-doc:
	$(CARGO) test --workspace --doc

fmt:
	$(CARGO) fmt --all

fmt-check:
	$(CARGO) fmt --all -- --check

lint:
	$(CARGO) clippy --workspace --all-targets -- -D warnings

doc:
	RUSTDOCFLAGS="$(RUSTDOCFLAGS)" $(CARGO) doc --workspace --no-deps

coverage:
	$(CARGO) llvm-cov --workspace --fail-under-lines 100

audit:
	$(CARGO) audit --deny warnings \
		--ignore RUSTSEC-2024-0436 \
		--ignore RUSTSEC-2025-0141

deny:
	$(CARGO) deny check

release:
	$(CARGO) build --release --workspace

contracts-check:
	pv lint contracts

bashrs-lint:
	bashrs lint Makefile

comply:
	pmat comply

ship-ready: fmt-check lint doc test test-doc coverage audit deny release contracts-check bashrs-lint comply
	@echo ""
	@echo "ship-ready: all gates green"

clean:
	$(CARGO) clean
